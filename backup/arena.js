...
import { multipleChoiceProblems, arenaGhostRecords } from '../problem-data.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchMatchAndStartGame(matchId) {
        try {
            const matchData = await performMockMatchmaking(matchId);
    ...
    async function createNewMatchAndRedirect() {
        ...
        window.location.href = `/arena.html?matchId=${newMatchId}`;
    }

    // ▼▼▼ 누락되었던 함수 ▼▼▼
    async function performMockMatchmaking(matchId) {
        console.log(`매치 ID(${matchId})에 대한 데이터 로딩 시뮬레이션...`);
        if (!multipleChoiceProblems || !arenaGhostRecords || arenaGhostRecords.length === 0) {
            throw new Error("문제/고스트 데이터가 없습니다.");
        }
        ...
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                ...
                let correspondingProblem = null;
                let attempts = 0;

                // 적합한 매칭을 찾을 때까지 최대 10번 시도 (무한 루프 방지)
                while (!selectedGhost && attempts < 50) {
                    // 1. 전체 문제 목록에서 무작위로 문제 하나를 먼저 선택합니다.
                    const randomProblem = { ...multipleChoiceProblems[(Math.random() * multipleChoiceProblems.length) | 0] };

                    // 2. 해당 문제를 푼, 내가 아닌 고스트 기록들만 모두 필터링합니다.
                    const userRating = currentUserProfile.rating;
                    const ratingDifferenceLimit = 100; // 허용할 레이팅 차이 범위 (예: 100점)

                    const potentialOpponents = arenaGhostRecords.filter(g => {
                        // 상대방과의 레이팅 차이 계산
                        const ratingDifference = Math.abs(userRating - g.rating);

                        return g.problemId === randomProblem.id && 
                            g.nickname !== currentUserProfile.nickname &&
                            ratingDifference <= ratingDifferenceLimit; // 레이팅 차이가 100 이하인 조건 추가
                    });

                    // 3. 만약 이 문제를 푼 상대가 있다면, 매칭 로직을 진행합니다.
                    if (potentialOpponents.length > 0) {
                        // 4. 필터링된 고스트들 중에서, 내 레이팅과 가장 가까운 상대를 찾습니다.
                        let bestMatch = potentialOpponents[0];
                        let minRatingDiff = Math.abs(currentUserProfile.rating - bestMatch.rating);

                        for (let i = 1; i < potentialOpponents.length; i++) {
                            const ghost = potentialOpponents[i];
                            const currentDiff = Math.abs(currentUserProfile.rating - ghost.rating);
                            if (currentDiff < minRatingDiff) {
                                minRatingDiff = currentDiff;
                                bestMatch = ghost;
                            }
                        }
                        
                        selectedGhost = bestMatch;
                        correspondingProblem = randomProblem;
                    }
                    
                    attempts++;
                }
                
                // 5. 최종적으로 매칭이 성공했다면, 데이터를 반환합니다.
                if (selectedGhost && correspondingProblem) {
                    resolve({ problem: correspondingProblem, opponent: { ...selectedGhost } });
                } else {
                // 6. 10번을 시도해도 적절한 상대를 못 찾았다면, 오류를 발생시킵니다.
                    reject(new Error("적합한 대전 상대를 찾는 데 실패했습니다. 잠시 후 다시 시도해주세요."));
                }
    ...
    async function showResult(resultText) {
        ...
        // --- 레이팅 변동 표시 (Mock 데이터) ---
        // let ratingChange = 0;
        // if (resultText === '승리!') ratingChange = Math.floor(Math.random() * 5 + 10);
        // else if (resultText === '패배!') ratingChange = -Math.floor(Math.random() * 5 + 10);
        ...
        // 나의 레이팅 변화 계산
        ratingChange = (resultText === '무승부') ? 0 : calculateEloChange(currentUserProfile.rating, opponentRecord.rating, myGameOutcome);
